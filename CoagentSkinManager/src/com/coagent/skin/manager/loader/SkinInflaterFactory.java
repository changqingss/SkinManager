...
public class SkinInflaterFactory implements Factory {
	...
	public View onCreateView(String name, Context context, AttributeSet attrs) {
	    ...
	    if (DEBUG) SkinLog.i("SkinInflaterFactory", "onCreateView: " + name);
	...
	private void parseSkinAttr(Context context, AttributeSet attrs, View view) {
        if (DEBUG) SkinLog.d("SkinInflaterFactory", "parseSkinAttr()");
		List<SkinAttr> viewAttrs = new ArrayList<SkinAttr>();
		for (int i = 0; i < attrs.getAttributeCount(); i++){
			String attrName = attrs.getAttributeName(i);
			String attrValue = attrs.getAttributeValue(i);
			
			if (DEBUG) SkinLog.d("SkinInflaterFactory", "attrName = " + attrName
			        + ", attrValue = " + attrValue);
		    ...
		    if(attrValue.startsWith("@")){
				try {
					...
					if (DEBUG) SkinLog.d("SkinInflaterFactory", "mSkinAttr = " + mSkinAttr);
					if (mSkinAttr != null) {
		...
		if(!ListUtils.isEmpty(viewAttrs)){
			...
			if (DEBUG) SkinLog.d("SkinInflaterFactory", "skinItem = " + skinItem);
